...
1. Mission Flow & Phases

    Phase 1: The Silent Approach
    The player spawns roughly N nautical miles from the yacht. The yacht is stationary. The trick in this mission is EMCON. Navigating by ESM only, they must chart a course that weaves between the patrol routes of 3-4 Coast Guard vessels.
...
2. Under-the-Hood: Editor Logic & Triggers
...
To build this in the Sea Power .ini files, use the following trigger structure:
    ...
    Trigger: Initiate VIP Transfer
        ...
        Action: Timer Start (120 seconds). (check CLAUDE.md for how to implement timers in triggers).
